...
def build_model(unfreeze):
    '''Takes in int value for number of total layers to unfreeze (last three layers are classifier)'''
...
def set_callbacks(red_patience, early_patience):
    ...
    mc = ModelCheckpoint(filepath = './Models/fine_tune_weights.{epoch:02d}-{val_loss:.2f}.hdf5',
                        monitor='val_loss',
                        verbose=2,
                        save_best_only=True,
...
def model_compile_weights(weights_filepath ,model, LR=0.000001, loss_='categorical_crossentropy', metrics_=['accuracy']):
...
def graph_history(metrics, loss_type, hist):
    ...
    for i in metrics:
        ...
        plt.savefig('fine_tune_'i+'_graph.png')
        ...
        plt.xlabel('epoch')
        plt.legend(['train', 'val'], loc='upper left')
        plt.savefig('fine_tune_loss_graph.png')
...
def main():
    ...
    model = model_compile_weights(model)
